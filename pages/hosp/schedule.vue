    ...
    <div class="nav-container page-component">
      ...
      <div class="nav left-nav">
        <div class="nav-item selected">
          <span class="v-link selected dark" :onclick="'javascript:window.location=\'/hosp/'+hoscode+'\''">Appointment </span>
        </div>
        <div class="nav-item ">
        ...
        <div class="nav-item "><span
          class="v-link clickable dark"> Holiday Notice </span>
        </div>
        <div class="nav-item "><span
          class="v-link clickable dark"> Search/Cancel </span>
        </div>
      </div>
      <!-- 左侧导航 #end -->
      <!-- 右侧内容 #start -->
      <div class="page-container">
        <div class="hospital-source-list">
          ...
          <div class="mt60">
            ...
            <div class="calendar-list-wrapper">
              ...
              <div :class="'calendar-item '+item.curClass" style="width: 124px;"
                ...
                <div class="status-wrapper" v-if="item.status == 0">{{ item.availableNumber == -1 ? 'Not Available' : item.availableNumber == 0 ? '约满' : 'Available' }}</div>
          ...
          <div class="countdown-wrapper mt60" v-if="!tabShow">
            <div class="countdonw-title"> {{ time }}<span class="v-link selected">{{ baseMap.releaseTime }} </span>Release</div>
          ...
          <div class="mt60" v-if="tabShow">
            <div class="">
              <div class="list-title">
                <div class="block"></div>
                上午号源
              </div><!-- eslint-disable vue/no-use-v-if-with-v-for,vue/no-confusing-v-for-v-if -->
              ...
              <div v-for="item in scheduleList" :key="item.id" v-if="item.workTime == 1">
                <div class="list-item">
                  ...
                  <div class="right-wrapper">
                    ...
                    <div class="button-wrapper">
                      <div class="v-button" @click="booking(item.id, item.availableNumber)" :style="item.availableNumber == 0 || pageFirstStatus == -1 ? 'background-color: #7f828b;' : ''">
                        <span>Remain<span class="number">{{ item.availableNumber }}</span></span></div>
                    </div>
                  </div>
                </div>
              </div>
    ...
    methods: {
      ...
      countDown(releaseTime, nowTime) {
        ...
        this.timer = setInterval(() => {
          secondes = secondes - 1
          if(secondes <= 0) {
            clearInterval(timer); //eslint-disable-line no-undef
            this.init()
          }
