    ...
    <div class="nav-container page-component">
        ...
        <div class="content-wrapper">
            ...
            <el-form-item prop="phone" label="手机号码：">
                <el-input v-model="patient.phone" placeholder="请输入手机号码" maxlength="36" class="input v-input"/>
            </el-form-item>
            </el-form>
        </div>
        <div class="sub-title">
...
export default {
        ...
        return {
            ...
            props: {
                ...
                dictApi.findByParentId(level ? node.value : '86').then(response => {
                    ...
                    let provinceList = list.map((item, i) => { //eslint-disable-line no-unused-vars
                        return {
                            value: item.id,
                            label: item.name,
    ...
    methods: {
        ...
        saveData() {
            ...
            this.submitBnt = '正在提交...'
                patientApi.save(this.patient).then(response => { //eslint-disable-line no-unused-vars
        ...
        updateData() {
            ...
            this.submitBnt = '正在提交...'
            patientApi.updateById(this.patient).then(response => { //eslint-disable-line no-unused-vars
                this.$message.success("提交成功")
                window.location.href = '/patient'
            }).catch(e => { //eslint-disable-line no-unused-vars
                this.submitBnt = '保存'
